...
基础配置现已采用表格形式展示，支持弹窗编辑：
- 服务地址：Alist 服务的访问地址
- 用户名：Alist 管理员账号
- 密码：Alist 管理员密码
- 令牌：Alist 令牌
- **连接测试**：支持在保存前测试基础连接是否正常。

### 2. 通知配置

通知配置支持多通道管理：
- **名称与类型**：支持自定义通道名称，当前支持 Bark 类型。
- **状态管理**：支持一键启用或禁用特定的通知通道。
- **推送测试**：在保存前可进行推送测试，确保配置正确。
- **灵活性**：支持添加多个通知通道，若无启用通道则不发送推送。

### 3. 同步任务配置

同步任务管理更加直观：
- **同步路径**：表格中直接展示源存储路径到目标存储路径的映射。
- **操作整合**：每个任务均有独立的编辑、删除和运行按钮。
- **全局操作**：页面底部提供“立即运行全部任务”功能。
...
数据存储已从 JSON 文件迁移至 **SQLite** 数据库，确保了数据的可靠性和并发能力。
- 数据库文件：`data/data.db`
- 首次运行会自动进行数据迁移（从旧的 JSON 文件迁移到 SQLite）。
...
- 2026-01-09
- **重大更新：数据库重构**
  - 使用 SQLite 替代 JSON 文件存储配置数据，提升稳定性和性能。
  - 自动完成从 JSON 到 SQLite 的数据迁移。
- **通知系统升级**
  - 支持多通道通知管理（如 Bark）。
  - 支持通知通道的启用/禁用和独立测试功能。
- **UI 深度重构**
  - 全面采用表格化展示，管理更清晰。
  - 所有配置（基础配置、通知配置、同步任务）均采用弹窗（Modal）形式进行新增和修改。
  - 任务列表展示同步路径摘要。
  - 优化移动端适配和交互体验。
- **功能优化**
  - 修复了 `runTask` 命名冲突导致的立即执行无效问题。
  - 增强了连接测试功能。
  - 新增“运行全部任务”功能。
  - 代码注释全面中文化，方便二次开发。
